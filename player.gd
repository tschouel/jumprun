...
@onready var walking_sprite: Node2D = get_node_or_null("Walking")
@onready var sliding_sprite: Node2D = get_node_or_null("Sliding")
...
func _check_default_sprite_state() -> void:
	...
	if is_fermata_active:
		...
		if not sliding_sprite or not is_instance_valid(sliding_sprite):
			sliding_sprite = get_node_or_null("Sliding")
