...
##
## Fuer kontinuierliches Greifen (z.B. beim Fader, wo sich das Ziel waehrend
## des Haltens bewegt) gibt es zusaetzlich extend_and_hold(target) /
## update_target(target) / retract() - siehe unten.
##
...
## Wie lange kurz am Ziel "gehalten" wird, bevor es sich zurueckzieht (nur bei reach_and_retract).
...
## Faehrt zum Zielpunkt aus und BLEIBT dort (kein automatisches Zurueckziehen).
## Fuer kontinuierliches Greifen wie beim Fader: danach waehrend des Haltens
## laufend update_target() aufrufen, damit die Spitze einem sich bewegenden
## Ziel folgt, und am Ende retract() zum Loslassen.
func extend_and_hold(target: Node2D) -> void:
	if not target:
		return
	_target_pos = target.global_position
	if _tween and _tween.is_valid():
	...
	visible = true
	_tween = create_tween()
	_tween.tween_method(_set_reach_t, _reach_t, 1.0, extend_duration).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
## Aktualisiert nur die Zielposition, ohne die Greif-Animation neu zu starten -
## damit die Armspitze einem sich bewegenden Ziel (z.B. Fader-Griff) folgt.
## Jeden Frame waehrend des Haltens aufrufen.
func update_target(target: Node2D) -> void:
	if not target:
		return
	_target_pos = target.global_position
	queue_redraw()
## Zieht die Elipse wieder ein (Gegenstueck zu extend_and_hold).
func retract() -> void:
	if _tween and _tween.is_valid():
		_tween.kill()
	_tween = create_tween()
	_tween.tween_method(_set_reach_t, _reach_t, 0.0, retract_duration).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
	_tween.tween_callback(func() -> void: visible = false)
