...
func setup(p_player: CharacterBody2D) -> void:
	...
	var flying_sprite = _get_flying_sprite()
	if flying_sprite:
...
func set_active(active: bool) -> void:
	...
	if flying_sprite:
		if active:
			...
			if flying_sprite.sprite_frames and flying_sprite.sprite_frames.has_animation("default"):
				flying_sprite.play("default")
		else:
			flying_sprite.visible = false
			flying_sprite.stop()
			flying_sprite.rotation = 0.0
...
func _trigger_boost() -> void:
	...
	var flying_sprite = _get_flying_sprite()
	if flying_sprite:
		if flying_sprite.sprite_frames and flying_sprite.sprite_frames.has_animation("boost"):
...
func _on_flying_sprite_animation_finished() -> void:
	var flying_sprite = _get_flying_sprite()
	if player and player.is_flying_active and flying_sprite and flying_sprite.animation == "boost":
		flying_sprite.speed_scale = fly_anim_base_speed
		flying_sprite.play("default")

func _get_flying_sprite() -> AnimatedSprite2D:
	var sprite = get_node_or_null("FlyingSprite") as AnimatedSprite2D
	if not sprite and player:
		sprite = player.get_node_or_null("FlyingSprite") as AnimatedSprite2D
	return sprite
